...
export const events = createCollection({
	name: "events",
	directory: "./content/de/events/",
	include: ["*/index.mdx"],
	read(item) {
		return reader.collections["de:events"].readOrThrow(item.id, { resolveLinkedFiles: true });
	},
	async transform(data, item, context) {
		...
		const summary = await transformMdxField(metadata.summary);
