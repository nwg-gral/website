...
function createContactPageCollection<TLocale extends IntlLocale>(locale: TLocale) {
	const language = getIntlLanguage(locale);
	const collection = withI18nPrefix("contact-page", language);
	...
	return createCollection({
		name: collection,
		directory: `./content/${language}/contact-page/`,
		include: ["index.mdx"],
		read() {
...
export const contactPage = {
	de: createContactPageCollection("de-DE"),
	en: createContactPageCollection("en-GB"),
};
